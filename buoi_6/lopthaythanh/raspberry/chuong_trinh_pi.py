"""
Buoi 6 - Chuong trinh Raspberry Pi
De bai: muc do 3 cua buoi 5 (lop thay Thanh), giu nguyen moi dieu kien van
hanh, doi sang bo channel/API key moi va them kha nang tu phuc hoi.

VI SAO TACH 2 CHANNEL (do that tren phan cung):
    ThingSpeak gioi han toi thieu ~17 GIAY giua 2 lan ghi len CUNG 1 channel
    (tai lieu ghi 15s nhung do thuc te chat hon), trong khi de bai bat buoc Pi
    gui trung binh cam bien moi 20 GIAY. Neu de chung 1 channel thi moi chu ky
    21s chi con ~4s trong cho lenh nut bam -> lenh HTTP tu Web bi tu choi lien
    tuc. Tach ra 2 channel thi channel LENH luon ranh -> bam nut an ngay (<2s).

Chuc nang (theo de muc do 3):
- Doc nhiet do, do am (DHT), dien ap bien tro (ADC), khoang cach (sieu am)
  moi 1 giay; trung binh moi 20 giay va gui 1 goi tin len CHANNEL CAM BIEN
  qua HTTP (field1..field4), lien tuc toi thieu 45 phut.
- Du lieu ngoai dai hop le bi loai, KHONG gui len Server (de bai: "neu du lieu
  cam bien bi loi thi khong duoc gui len Server ma phai doc lai va gui du lieu
  khac").
- LCD 16x2 hien thi thoi gian hien tai + che do + trang thai 3 thiet bi.
- Nhan lenh dieu khien tu Web tren CHANNEL LENH (field1..field4):

    Auto        -> MQTT   (field4 = 0)  | de bai: NUT CHON CHE DO
    Manual      -> MQTT   (field4 = 1)  | bat buoc dung MQTT
    ----------------------------------- +
    LED Bat     -> HTTP   (field1 = 1)  |
    LED Tat     -> HTTP   (field1 = 0)  |
    Relay Bat   -> HTTP   (field2 = 1)  | de bai: NUT DIEU KHIEN
    Relay Tat   -> HTTP   (field2 = 0)  | bat buoc dung HTTP
    Buzzer Bat  -> HTTP   (field3 = 1)  |
    Buzzer Tat  -> HTTP   (field3 = 0)  |

  Ca 8 nut deu do WEB (Node-RED) ghi len CHANNEL LENH.

- Che do Auto (dung nguyen dieu kien cua de):
    LED    sang 18h-22h, tat khoang thoi gian con lai.
    Buzzer keu khi nhiet do > 40 do C, tat khi < 30 do C, giua giu nguyen.
    Relay  dong khi do am  > 70 %,     tat khi < 40 %,    giua giu nguyen.

- Che do Manual: LED/Buzzer/Relay theo 6 nut tren Web, trang thai doi cham
  nhat 2 giay ke tu khi du lieu nut nhan len Server thanh cong.

- Pi CHI DOC LENH QUA HTTP POLLING MOI GIAY, KHONG SUBSCRIBE MQTT.
  Ly do: ThingSpeak chi cap 1 bo danh tinh MQTT cho moi channel (client_id
  phai trung username). Neu ca Web va Pi cung mo ket noi bang chung client_id
  thi broker da nhau lien tuc, lam rot phan lon lenh publish. Vi vay chi Web
  giu ket noi MQTT; Pi doc bang HTTP polling - van nhan du ca 8 nut vi
  ThingSpeak luu chung moi lan ghi (du giao thuc nao) vao cung 1 feed.

TINH NANG THEM CUA BUOI 6:
- Tu khoi dong khi bat nguon Raspberry: chay bang systemd service
  (buoi_6/systemd/iot-buoi6.service, da 'systemctl enable').
- Tu khoi dong lai khi loi keo dai: loi le te thi try/except nuot va chay tiep
  (dung goi y cua de), nhung neu cam bien hong han hoac rot mang lien tiep qua
  nguong thi thoat voi ma loi de systemd dung lai mot tien trinh sach.

So do noi day (GPIO / cong Grove):
...
MIN_RUN_SECONDS = 45 * 60  # chay lien tuc toi thieu 45 phut (yeu cau de muc do 3)
...
SENSOR_CHANNEL_ID = "DIEN_CHANNEL_ID_CAM_BIEN_CUA_BAN"          # buoi_6_channel_1
SENSOR_WRITE_API_KEY = "DIEN_WRITE_API_KEY_CAM_BIEN_CUA_BAN"

# --- Channel LENH: chi chua 8 nut dieu khien, Web ghi (MQTT + HTTP) ---
COMMAND_CHANNEL_ID = "DIEN_CHANNEL_ID_LENH_CUA_BAN"         # buoi_6_channel_2
...
FIELD_LED = "field1"       # Web ghi HTTP (ca Bat lan Tat)
FIELD_RELAY = "field2"     # Web ghi HTTP (ca Bat lan Tat)
FIELD_BUZZER = "field3"    # Web ghi HTTP (ca Bat lan Tat)
FIELD_MODE = "field4"      # Web ghi MQTT (bat buoc theo de): 0 = Auto, 1 = Manual
...
# ---------------------------------------------------------------------------
# TU PHUC HOI (tinh nang them cua buoi 6)
#
# De bai goi y dung try/except de khi cam bien loi hoac rot mang thi chuong
# trinh tu dong chay tiep, KHONG can khoi dong lai - phan do van giu nguyen o
# cac lop try/except ben duoi. Nhung neu loi keo dai LIEN TIEP (cam bien hong
# han, mat mang lau) thi chay tiep cung khong con y nghia: chuong trinh se
# thoat voi ma loi 1 de systemd (iot-buoi6.service, Restart=always) dung lai
# mot tien trinh hoan toan sach.
#
# Moi lan thanh cong deu reset bo dem ve 0, nen chi loi LIEN TIEP moi tinh.
# ---------------------------------------------------------------------------
SENSOR_FAIL_LIMIT = 30   # ~30 giay lien tiep khong doc duoc BAT KY cam bien nao
NETWORK_FAIL_LIMIT = 20  # 20 lan lien tiep goi ThingSpeak that bai

health = {'sensor_fail': 0, 'network_fail': 0}


def cleanup_outputs():
    """Tat het 3 thiet bi truoc khi thoat.

    GPIO khong tu ve muc thap khi tien trinh ket thuc - neu dang bat ma thoat
    ngang thi den/coi/relay se giu nguyen trang thai vat ly.
    """
    for device in (led, buzzer, relay):
        try:
            device.off()
        except Exception:
            pass
...
def fail_exit(reason):
    """Thoat han tien trinh de systemd dung lai.

    PHAI dung os._exit() chu khong phai sys.exit(): ham nay co the duoc goi tu
    LUONG DOC LENH (control_poll_loop). sys.exit() trong luong phu chi ket thuc
    dung luong do roi tien trinh chay tiep trong trang thai que quat (khong con
    doc lenh nua) - dung sai lam. os._exit() ket thuc ca tien trinh tu bat ky
    luong nao. Vi os._exit() bo qua khoi finally nen phai tu tat thiet bi truoc.

    Buoc don dep nam trong try/finally de DU CO LOI GI di nua thi van thoat.
    Neu khong lam vay, mot ngoai le o day se bi 'except Exception' cua vong lap
    goi no nuot mat, tien trinh chay tiep trong trang thai hong ma khong ai
    biet - da gap dung loi nay khi test (thieu 'import sys' -> NameError bi
    nuot -> dem loi cu tang 21/20, 22/20... ma khong bao gio khoi dong lai).
    """
    try:
        log_event(f"TU KHOI DONG LAI: {reason} -> thoat de systemd dung lai tien trinh")
        cleanup_outputs()
        sys.stdout.flush()
    finally:
        os._exit(1)
...
def note_sensor_ok():
    health['sensor_fail'] = 0


def note_sensor_fail():
    health['sensor_fail'] += 1
    n = health['sensor_fail']
    print(f"[CAM BIEN] Khong doc duoc gia tri nao ({n}/{SENSOR_FAIL_LIMIT} lan lien tiep)")
    if n >= SENSOR_FAIL_LIMIT:
        fail_exit(f"cam bien loi {n} lan lien tiep")


def note_network_ok():
    health['network_fail'] = 0


def note_network_fail():
    health['network_fail'] += 1
    n = health['network_fail']
    print(f"[MANG] Goi ThingSpeak that bai ({n}/{NETWORK_FAIL_LIMIT} lan lien tiep)")
    if n >= NETWORK_FAIL_LIMIT:
        fail_exit(f"rot mang {n} lan lien tiep")
    ...
    try:
        while True:
            try:
                ...
                # Ca 4 cam bien deu khong ra gia tri hop le -> tinh 1 lan loi.
                # Chi can 1 cam bien doc duoc la coi nhu phan cung con song.
                if temp is None and humi is None and voltage is None and distance is None:
                    note_sensor_fail()
                else:
                    note_sensor_ok()
